# -*- coding: utf-8 -*-
"""
Created on Sun Sep 26 19:25:22 2021
...
'''
https://www.projectpro.io/recipes/do-recursive-feature-elimination-in-python

from sklearn.feature_selection import RFECV
rfecv = RFECV(estimator=RandomForestClassifier(), step=1, 
              scoring="f1_weighted", cv=4, verbose=0, n_jobs=4)
rfecv.fit(X_train_scaled, Y_train)
rfecv.transform(X_train_scaled)
print(rfecv)
print(rfecv.n_features_)  #15
...
# Fit the Random Forest with 300 Decision Trees
...
feature_scores_As = pd.Series(rf.feature_importances_, index=X_train.columns).sort_values(ascending=True)
f, ax = plt.subplots(figsize=(30, 24))
ax = plt.barh(feature_scores_As.index, feature_scores_As)
...
print("Training set score for RF: %f" % rfe.score(X_train_scaled, Y_train))
print("Testing  set score for RF: %f" % rfe.score(X_test_scaled, Y_test ))
