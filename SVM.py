# -*- coding: utf-8 -*-
"""
Created on Sun Sep 26 14:16:54 2021
...
# Choose the top 18 featues and use them for the model
n_features_to_select = 18
svc = SVC(kernel='linear', C=1000) 
rfe = RFE(svc, n_features_to_select=n_features_to_select)
...
Y_pred_label = list(encoder.inverse_transform(prediction))
...
print("Training set score for SVM: %f" % rfe.score(X_train_scaled, Y_train))
print("Testing  set score for SVM: %f" % rfe.score(X_test_scaled, Y_test ))
...
# Performing CV to tune parameters for best SVM fit 
svm_model_colon = GridSearchCV(SVC(), params_grid, cv=5)
svm_model_colon.fit(X_train_scaled_colon, Y_train_colon)

# View the accuracy score
print('Best score for training data:', svm_model_colon.best_score_,"\n") 

# View the best parameters for the model found using grid search
print('Best C:',svm_model_colon.best_estimator_.C,"\n") 
print('Best Kernel:',svm_model_colon.best_estimator_.kernel,"\n")
print('Best Gamma:',svm_model_colon.best_estimator_.gamma,"\n")

final_model_colon = svm_model_colon.best_estimator_
Y_pred_colon = final_model_colon.predict(X_test_scaled_colon)
...
print("Training set score for SVM: %f" % final_model_colon.score(X_train_scaled_colon , Y_train_colon))
print("Testing  set score for SVM: %f" % final_model_colon.score(X_test_scaled_colon  , Y_test_colon ))
...
# We will split our data. 80% of our data will be train data and 
# 20% of it will be test data.
X_train_lung, X_test_lung, Y_train_label_lung, Y_test_label_lung = train_test_split(X_lung,Y_lung,test_size = 0.2,random_state=0)
...
# Performing CV to tune parameters for best SVM fit 
svm_model_lung = GridSearchCV(SVC(), params_grid, cv=5)
svm_model_lung.fit(X_train_scaled_lung, Y_train_lung)

# View the accuracy score
print('Best score for training data:', svm_model_lung.best_score_,"\n") 

# View the best parameters for the model found using grid search
print('Best C:',svm_model_lung.best_estimator_.C,"\n") 
print('Best Kernel:',svm_model_lung.best_estimator_.kernel,"\n")
print('Best Gamma:',svm_model_lung.best_estimator_.gamma,"\n")

final_model_lung = svm_model_lung.best_estimator_
Y_pred_lung = final_model_lung.predict(X_test_scaled_lung)
...
print("Training set score for SVM: %f" % final_model_lung.score(X_train_scaled_lung , Y_train_lung))
print("Testing  set score for SVM: %f" % final_model_lung.score(X_test_scaled_lung  , Y_test_lung ))
