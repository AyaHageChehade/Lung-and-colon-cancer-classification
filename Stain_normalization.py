...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 11 11:44:11 2022
...
# path
path_colon_aca = glob.glob("/home/anaconda/Downloads/LC25000/lung_colon_image_set/colon_image_sets/colon_aca/*.jpeg")
path_colon_n = glob.glob("/home/anaconda/Downloads/LC25000/lung_colon_image_set/colon_image_sets/colon_n/*.jpeg")
path_lung_aca = glob.glob("/home/anaconda/Downloads/LC25000/lung_colon_image_set/lung_image_sets/lung_aca/*.jpeg")
path_lung_n = glob.glob("/home/anaconda/Downloads/LC25000/lung_colon_image_set/lung_image_sets/lung_n/*.jpeg")
path_lung_scc = glob.glob("/home/anaconda/Downloads/LC25000/lung_colon_image_set/lung_image_sets/lung_scc/*.jpeg")
...
def normalize(images):
    img_Norm = []
    i1 = staintools.read_image("/home/anaconda/Downloads/LC25000/colonca2.jpeg")
    for imgnorm in images:
        if STANDARDIZE_BRIGHTNESS:
            # Standardize brightness
            i1 = staintools.LuminosityStandardizer.standardize(i1)
            imgnorm = staintools.LuminosityStandardizer.standardize(imgnorm)
        # Stain normalization
        normalizer = staintools.StainNormalizer(method=METHOD)
        normalizer.fit(i1)
        imgnorm_normalized = normalizer.transform(imgnorm)
        #plt.imshow(imgnorm_normalized, cmap=plt.cm.gray)
        #plt.show()
        img_Norm.append(imgnorm_normalized)     
    return(img_Norm)

img_Masque_colon_aca = normalize(img_colon_aca) 
img_Masque_colon_n = normalize(img_colon_n) 
img_Masque_lung_aca = normalize(img_lung_aca) 
img_Masque_lung_n = normalize(img_lung_n) 
img_Masque_lung_scc = normalize(img_lung_scc) 

# First order statistics
def mean(img_Norm):
    Mean = []    
    for imgMasque in img_Norm:
...
def matrice_co_occureence(img_Norm):
    matrix_coocurrence = []
    for Masque in img_Norm:
...
params_grid = [{'n_estimators': [10, 50, 100, 300], #default=10
            'criterion': ['gini', 'entropy'], #default=”gini”
            'max_depth':[2, 4, 6, 8, 10, None], #default=None
            'oob_score': [True]}]

# Performing CV to tune parameters for best  fit 
...
# View the best parameters for the model found using grid search
print('Best n_estimators:',rf_model.best_estimator_.n_estimators,"\n") 
print('Best criterion:',rf_model.best_estimator_.criterion,"\n")
print('Best max_depth:',rf_model.best_estimator_.max_depth,"\n")
print('Best oob_score:',rf_model.best_estimator_.oob_score,"\n")
...
# Check accuracy score 
print('Model accuracy score  : {0:0.4f}'. format(accuracy_score(Y_test_label, Y_pred_label)))
...
print("Training set score for RF: %f" % rf_model.score(X_train_scaled , Y_train))
print("Testing  set score for RF: %f" % rf_model.score(X_test_scaled  , Y_test ))
...
X_lda = lda_model.fit_transform(X_train_scaled, Y_train)
...
print("Training set score for LDA: %f" % lda_model.score(X_train_scaled , Y_train))
print("Testing  set score for LDA: %f" % lda_model.score(X_test_scaled  , Y_test ))
...
print("Training set score for LightGBM: %f" % model.score(X_train_scaled, Y_train))
print("Testing  set score for LightGBM: %f" % model.score(X_test_scaled, Y_test ))
